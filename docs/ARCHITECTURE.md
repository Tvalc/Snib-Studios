...
| Module | Responsibilities | Tech/Integration |
|--------|------------------|------------------|
| Code Editor (Front-end) | UI for coding, sends AI queries, shows suggestions; Git operations UI; game preview. | React/TypeScript UI, Monaco editor, communicates with AI Service and GitHub API. |
| AI Design/Code Assistant | Processes design questions and code prompts, returns generated code or ideas. | Runs on server, calls LLMs (e.g., OpenAI GPT-4). Handles context management. |
| Template Library | Stores and serves starter templates for games. | Could be static files or stored in DB/Git; accessible via API. |
| Project Mgmt & Docs Service | Generates project plans, architecture docs, and diagrams; tracks progress. | Uses AI for docs (could use Mermaid for diagrams). Stores task lists in DB. |
| GitHub Integration | Version control operations (init repo, commit, push, branch, PRs, collaborators). | GitHub REST API/GraphQL; uses OAuth tokens. |
| User Account & Data Backend | Authenticates users; stores user info, project data, settings. | Supabase (Postgres DB, Auth) or similar BaaS; integrates with Lovable auth. |
| Game Preview Engine | Runs the game for testing; provides logs/errors to user. | In-browser for HTML5 games (iframe), or server container for any backend logic. |
| Plugin Manager | Manages installation and sandboxing of extensions. | Plugin registry; secure sandbox environment (e.g., iframe or VM). |
...
| Aspect | Technology Choice | Reason |
|--------|-------------------|--------|
| Front-end UI | React + TypeScript + Tailwind CSS | Modern, fast UI, matches Lovable's tech for synergy. |
| Code Editor Component | Monaco (VSCode editor core) | Rich editing features (intellisense, syntax highlighting). |
| Back-end Framework | Node.js (Express/NestJS) or Python (FastAPI) | JavaScript synergy with front-end (Node); Python if leveraging ML directly. |
| Database & Auth | Supabase (PostgreSQL) + Clerk (Auth) | Scalable and integrates with Lovable easily; provides user auth and storage out-of-box. |
| AI Integration | OpenAI GPT-4 API (initially) | High-quality code and design generation; faster start than training new models. |
| Game Engine (2D) | Phaser JS framework | Well-known HTML5 game engine, good for platformers, etc. |
| Game Engine (3D) | Three.js or Babylon.js | Enables WebGL 3D games in browser (for FPS or 3D genres). |
| Version Control | Git (via GitHub API) | Industry standard VCS, GitHub integration for collaboration. |
| Deployment (optional) | Vercel/Netlify, or itch.io integration | Easy web deployment for finished games; itch.io for game distribution to community. |
